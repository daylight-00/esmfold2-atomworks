...
esmfold2-atomworks doctor      # upstreams, imports, weights; parity given AtomWorks' test data
...
- The ESMFold2 module ships in `esm` ≥ 3.4. Import from the **leaf** modules
  (`.types`, `.processor`, `.prepare_input`, `.conformers`), which avoid loading
  the model stack to featurize. (`load_native_model_class()` still recognises
  the esm ≤ 3.3 layout, a `transformers` fork that is no longer published.)
